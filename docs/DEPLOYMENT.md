...
`cloud-upload` verifies the locked hashes, adds the same source-row impression key used locally, and performs a `WRITE_TRUNCATE` snapshot load for each independent shard staging table. An attribution raw view unions six staging tables; a separate uplift view unions four checksum-verified source staging tables. The datasets are never joined. Retrying the complete upload replaces shards rather than appending duplicate events. Each load call uses its own job ID; the client handles retries of that call. The script records shard row counts and actual load job IDs. The supplementary Hillstrom cloud SQL is preserved but is not part of the default Criteo execution path.

`cloud-transform` transpiles the reviewed local SQL to GoogleSQL, qualifies only real table names, creates relative-day range partitions for impressions and campaign/day marts, clusters those tables by campaign/context, runs the seven analytical SQL modules, and executes native Criteo uplift arm/balance SQL. Confidence intervals, model fitting and static reporting remain Python/local components. The byte cap applies per query statement, not to the whole run, and is not a dollar budget. Staging tables are full snapshots; partitions support downstream queries, not an incremental source ingestion claim.
...
## SQL dialect differences and current orchestration

`render-cloud` translates DuckDB SQL with sqlglot, qualifies known warehouse tables and writes to `outputs/bigquery/`. It replaces `isfinite(cost)` with BigQuery IS_NAN/IS_INF guards. Exact lag quantiles use DuckDB aggregate MEDIAN/QUANTILE_CONT locally and native GoogleSQL PERCENTILE_CONT analytic windows plus DISTINCT in cloud. Relative day uses range partitions, not a fabricated calendar DATE. Example GoogleSQL is parser/round-trip validated, not service dry-run validated.

DAG: ingestion → validate source counts → cleaning → base attribution SQL → attribution/daily reconciliation → QA → measurement health → full Criteo experiment → uplift evaluation → optional BigQuery upload/transforms/QA → reporting. Tasks call reusable modules; one active run and sequential DuckDB writers prevent lock contention. No credentials or large frames are passed via XCom.

The local `run` command caches completed stages by source/file metadata and SQL/code fingerprints; ingestion always verifies SHA-256 before using files. Model sample and fit caches use source lock, settings and model code. Airflow tasks remain idempotent snapshot operations and reuse downloads/model fitting; they do not claim fully incremental historical ingestion. Partial failures stop downstream reporting; `run --force` rebuilds aggregate stages.

## GitHub Pages

`python scripts/save_artifacts.py` copies report/figures/aggregate CSVs into `artifacts/`, excluding raw data and row predictions. Push to `main` triggers the existing Pages workflow, which copies `report.html` to the site root `index.html`. Local static preview must serve the entire `artifacts/` folder so relative figure/CSV links resolve. The same published URL is retained.
